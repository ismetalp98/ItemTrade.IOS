...
extension DatabaseManager {
    ...
    public func createNewConversation(with otherUsername: String, itemId: String, otherUserId: String, firstMessage: Message, completion: @escaping (Bool) -> Void) {
        
        guard let currentNamme = UserDefaults.standard.value(forKey: "name") as? String else {
            return
        }
        let currentId = UserDefaults.standard.value(forKey: "userId") as! String
        
        let ref = database.child("Users/\(currentId)")
        ...
        ref.observeSingleEvent(of: .value, with: { [weak self] snapshot in
            ...
            self?.database.child("Users/\(otherUserId)/conversations").observeSingleEvent(of: .value, with: { [weak self] snapshot in
                if var conversatoins = snapshot.value as? [[String: Any]] {
                    ...
                    self?.database.child("Users/\(otherUserId)/conversations").setValue(conversatoins)
                }
                else {
                    // create
    ...
    public func getAllConversations(for userId: String, completion: @escaping (Result<[Conversation], Error>) -> Void) {
        database.child("Users/\(userId)/conversations").observe(.value, with: { snapshot in
            ...
            let conversations: [Conversation] = value.compactMap({ dictionary in
                ...
                return Conversation(id: conversationId,
                                    name: name,
                                    otherUserId: otherUserEmail, itemId: itemId,
    ...
    public func sendMessage(to conversation: String, itemId: String, otherUserId: String, name: String, newMessage: Message, completion: @escaping (Bool) -> Void) {
        ...
        database.child("Conversations/\(conversation)/messages").observeSingleEvent(of: .value, with: { [weak self] snapshot in
            ...
            strongSelf.database.child("Conversations/\(conversation)/messages").setValue(currentMessages) { error, _ in
                ...
                strongSelf.database.child("Users/\(myId)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                    ...
                    strongSelf.database.child("Users/\(myId)/conversations").setValue(databaseEntryConversations, withCompletionBlock: { error, _ in
                        guard error == nil else {
                            completion(false)
                            return
                        }
    ...
    public func conversationExists(with path: String, completion: @escaping (Result<String, Error>) -> Void) {
        
        let myId = UserDefaults.standard.value(forKey: "userId") as! String
        
        database.child("Users/\(myId)/conversations").observeSingleEvent(of: .value, with: { snapshot in
            ...
            if let conversation = collection.first(where:
                {
                    guard let targetSenderId = $0["id"] as? String else
                    {
                        return false
                    }
                    return path == targetSenderId
            })
            {
                // get id
...
extension DatabaseManager{
    ...
    public func getAllItems(completion: @escaping (Result<[Item], Error>) -> Void) {
        ...
        database.child("Itemsa").observe(.value, with: {snapshot in
            guard (snapshot.value as? [[String: Any]]) == nil else{
                completion(.failure(DatabaseError.failedToFetch))
                return
            }
            ...
            for child in snapshot.children{
                ...
                if let childSnapshots = child as? DataSnapshot,
                    ...
                    let urls = childSnapshots.childSnapshot(forPath: "urls").value as? [String:Any],
                    ...
                    for childURL in urls.values {
                        newUrlArray.append(childURL as! String)
                    }
                    let item = Item(title: title, content: content, itemId:  itemId, price: price, ownerId: ownerId, ownerName: ownerName, urls: newUrlArray)
